...
df_P1 <- read_excel("PATIENT_DATASET.xlsx")
...
# NUMBER OF WINDOWS OR NUMBER OF PREDICTIONS OF 12 TIME POINTS EACH
s_df = 288

# GENERATING DATA START AND END POINTS FOR EACH ITERATION
indexes <- array(integer(3 * s_df * 1), dim = c(3, s_df, 1))
dfs <- list(df_P1)
for(i in 1:s_df){
  indexes[1,i,1] <- i
  indexes[2,i,1] <- nrow(dfs[[1]]) - 300 + i
  indexes[3,i,1] <- nrow(dfs[[1]]) - 288 + i
}

# ASSIGNING INITIAL STATES AS PER QUANTILES OF THE GLUCOSE VALUES OF THE ENTIRE DATASET
for(m in 1:1){
  Quantiles <- quantile(unlist(df_P1[, 'CGM']), probs = c(0.15, 0.375, 0.625, 0.85))
  for(n in 1:nrow(df_P1)){
    if(is.na(df_P1[n, 'CGM'])){
      
    }
    else if(df_P1[n, 'CGM'] < Quantiles[1]){
      df_P1[n, 'State'] <- 1
    }
    else if(df_P1[n, 'CGM'] < Quantiles[2]){
      df_P1[n, 'State'] <- 2
    }
    else if(df_P1[n, 'CGM'] < Quantiles[3]){
      df_P1[n, 'State'] <- 3
    }
    else if(df_P1[n, 'CGM'] < Quantiles[4]){
      df_P1[n, 'State'] <- 4
    }
    else{
      df_P1[n, 'State'] <- 5
...
Tm_Pts <- 12
# 1 DATASET * 1 PREDICTION PER ITERATION * 12 TIMEPOINTS PER ITERATION * 288 ITERATIONS OR PREDICTIONS
Time_Point <- integer(1 * s_df * Tm_Pts)
State <- integer(1 * s_df * Tm_Pts)
CGM <- integer(1 * s_df * Tm_Pts)
Predicted_State <- integer(1 * s_df * Tm_Pts)
Hypo_Prob <- integer(1 * s_df * Tm_Pts)
Patient_Number <- integer(1 * s_df * Tm_Pts)
Actual_State_Com <- integer(1 * s_df * Tm_Pts)
Pred_State_Com <- integer(1 * s_df * Tm_Pts)
Prob_of_Hypo_30min <- integer(1 * s_df * Tm_Pts)
Prob_of_Hypo_60min <- integer(1 * s_df * Tm_Pts)
Time_Stamp <- as.character(integer(1 * s_df * Tm_Pts))
...
for(i in 1:(s_df)){
  ...
  df_train <- rbind(df_P1[indexes[1,i,1]:indexes[2,i,1],])
  ...
  Quantiles <- quantile(df_train$CGM, probs = c(0.15, 0.375, 0.625, 0.85))
  ...
  # 1 SET OF PREDICTION OF 12 TIMEPOINTS
  for(j in 1:1){
