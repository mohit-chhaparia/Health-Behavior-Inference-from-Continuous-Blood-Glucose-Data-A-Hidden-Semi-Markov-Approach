...
for (i in 1:20) { # Loop to read 20 datasets
  file_name <- sprintf(file_path_pattern, 1000 + i) # Creates the filename
  df <- read.csv(file_name) # Reads the file
  dfs[[i]] <- df # Assigns it to the datasets list
...
############################################################
# 20 DataFrames * 1 Predictions * (Tm_Pts + 1) TimePoints * s_df Subsets of each DataFrame
...
# dimension of indexes is 3 x 288 x 20
...
for(i in 1:s_df){
  ...
  # N is a vector indicating how many records are present from each patient's data set
